        ...
        <el-table-column  label="操作" width="220" fixed="right" >
          <template scope="scope" style="text-align:center" class="dobtn">
            <div scope="scope" class="dobox">
              <el-button size="small" icon="edit" type="info">分配</el-button>
              <el-row>【xinqian】正在操作<span class="lockstatus">解锁</span></el-row>
            </div>
          </template>
        </el-table-column>
      <template slot="foot">
...
<script>
import VerifyAssign from './formData/verifyAssign'
...
export default {
  ...
  computed: {
    columns () {
      return VerifyAssign.VerifyAssign
    }
  },
  methods: {
    // 获取表单数据
    handleGetTableData() {
      ...
      fn.handleGetTableData(api.risk.assignList, that.para, res => {
    ...
    batchRemove() {
      ...
      this.$confirm('确认删除选中记录吗？', '提示', {
        ...
        fn.handleRemoveTableData(api.risk.assignRemove, ids, (res) => {
           that.loading = false
           this.handleGetTableData()
        })
      }).catch(() => {
