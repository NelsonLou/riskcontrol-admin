const businessFirst = (mock, axios, qs) => {
    let data = mock.mock({
        'business|97': [
            {
              'id': '@increment',
    ...
    // 赋值新变量
    let BusinessFirst = data.business

    // 业务列表数据处理
    axios.onGet('/first-list').reply(config => {
      let {limit, page, filters} = config.params
      let MockBusiness = BusinessFirst.filter(L => {
  ...
  // 批量删除产品类型
  axios.onPost('/first-remove').reply(config => {
    let id = qs.parse(config.data).params
    BusinessFirst = BusinessFirst.filter(u => !id.includes(u.id))
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        resolve([
          200, {
